...
from config import  DB_FILE,SYMBOL,SEGMENT, CANDLE_DAYS as DAYS, REQUIRED_CANDLES, LOG_FILE,INSTRUMENTS_FILE, OPTION_SYMBOL, SERVER
import os
    ...
    try:
        ...
        sql = """
            INSERT INTO trade_config (
                ...
                EXPIRY, STRATEGY, CRT_DT, LST_UPDT_DT, ROLLOVER
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        params = (
...
def save_open_position(trade, config, tradeGenie_id):
    try:
        ...
        c = conn.cursor()
        sql = """
            INSERT INTO open_trades (signal, spot_entry, option_symbol, strike, expiry, option_sell_price, entry_time, qty, interval, real_trade, entry_reason, expiry_type, strategy, key, user_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        params = (
            ...
            ,trade.get("EntryReason","MANUAL_ENTRY"), trade.get("ExpiryType",config['EXPIRY']), trade.get("Strategy",config['STRATEGY']), trade.get("Key","NA"), tradeGenie_id
        )
...
def load_open_position(config,key , user , tradeGenie_id):
    ...
    sql = """
        SELECT signal, spot_entry, option_symbol, strike, expiry, 
               option_sell_price, entry_time, qty, interval, real_trade, entry_reason, expiry_type, strategy, key, user_id
...
def record_trade(trade, config, tradeGenie_id):
    ...
    sql = """
        ...
        entry_time, spot_exit, option_buy_price, exit_time, pnl, qty, interval, real_trade, entry_reason, exit_reason, expiry_type, strategy, key, user_id)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """
    params = (
        trade["Signal"], trade["SpotEntry"], trade['OptionSymbol'], trade["Strike"],
...
def close_position_and_no_new_trade(trade, position, close, ts, config, user):
    now = datetime.datetime.now()
    trade.update({
        "SpotExit": close,
        "ExitTime": ts,
    ...
    print(f"📥 {user['user']} {SERVER}  {config['INTERVAL']} Exiting BUY: Buying back {trade['OptionSymbol']} | Qty: {trade['qty']}")
    ...
    send_telegram_message(f"⏰{user['user']} {SERVER} {config['key']} TRADE close {position} \n{trade['OptionSymbol']} @ ₹{trade['OptionBuyPrice']:.2f}")
    return {}, None
